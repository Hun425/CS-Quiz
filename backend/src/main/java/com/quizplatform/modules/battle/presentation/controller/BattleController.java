package com.quizplatform.modules.battle.presentation.controller;

import com.quizplatform.core.config.security.UserPrincipal;
import com.quizplatform.modules.battle.presentation.dto.BattleRoomCreateRequest;
import com.quizplatform.modules.battle.presentation.dto.BattleRoomResponse;
import com.quizplatform.modules.battle.presentation.dto.BattleJoinResponse;
import com.quizplatform.core.dto.common.CommonApiResponse;
import com.quizplatform.core.exception.BusinessException;
import com.quizplatform.core.exception.ErrorCode;
import com.quizplatform.modules.battle.application.service.BattleService;
...
public class BattleController {
    ...
    public ResponseEntity<CommonApiResponse<BattleRoomResponse>> createBattleRoom(
        ...
        if (userPrincipal == null) {
            // TODO: Use shared_kernel exception/error code
            throw new BusinessException(ErrorCode.UNAUTHORIZED, "인증이 필요합니다.");
        }

        // Controller에서는 Request DTO를 Application Service로 전달
        // creatorId 설정은 Service 내부 또는 DTO 매핑 시 처리 고려
        BattleRoomResponse battleRoom = battleService.createBattleRoom(
                userPrincipal.getUser().getId(), // Pass only userId
                request // Pass the request DTO directly
        );

        // TODO: Use shared_kernel response wrapper
     ...
     * @return 활성화된 배틀방 목록
     */
    @Operation(summary = "활성화된 대결방 목록 조회", description = "현재 활성화된(대기중) 대결방 목록을 조회합니다.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<CommonApiResponse<List<BattleRoomResponse>>> getActiveBattleRooms() {
        // TODO: Ensure BattleRoomStatus enum is in battle module
        List<BattleRoomResponse> battleRooms = battleService.getBattleRoomsByStatus(BattleRoomStatus.WAITING);
        // TODO: Use shared_kernel response wrapper
    ...
    public ResponseEntity<CommonApiResponse<BattleRoomResponse>> joinBattleRoom(
        ...
        BattleRoomResponse battleRoom = battleService.joinBattleRoom(roomId, userPrincipal.getUser().getId()); // Pass only userId
        // TODO: Use shared_kernel response wrapper
        return ResponseEntity.ok(CommonApiResponse.success(battleRoom));
    }
    ...
    public ResponseEntity<CommonApiResponse<BattleRoomResponse>> toggleReady(
        ...
        BattleRoomResponse battleRoom = battleService.toggleReady(roomId, userPrincipal.getUser().getId()); // Pass only userId
        // TODO: Use shared_kernel response wrapper
        return ResponseEntity.ok(CommonApiResponse.success(battleRoom));
    }
    ...
    public ResponseEntity<CommonApiResponse<BattleRoomResponse>> leaveBattleRoom(
        ...
        BattleRoomResponse battleRoom = battleService.leaveBattleRoom(roomId, userPrincipal.getUser().getId()); // Pass only userId
        // TODO: Use shared_kernel response wrapper
        return ResponseEntity.ok(CommonApiResponse.success(battleRoom));
    }
    ...
    public ResponseEntity<CommonApiResponse<BattleJoinResponse>> getBattleParticipants(
            @Parameter(description = "대결방 ID") @PathVariable Long roomId) {

        // TODO: Ensure BattleJoinResponse is in battle module DTOs
        BattleJoinResponse participants = battleService.getCurrentBattleParticipants(roomId);
        // TODO: Use shared_kernel response wrapper
    ...
    public ResponseEntity<CommonApiResponse<BattleRoomResponse>> getMyActiveBattleRoom(
        ...
        BattleRoomResponse battleRoom = battleService.getActiveBattleRoomByUser(userPrincipal.getUser().getId()); // Pass only userId

        // TODO: Use shared_kernel response wrapper
        if (battleRoom == null) {
            // 활성 방이 없는 경우 성공 응답에 null 데이터 반환
            return ResponseEntity.ok(CommonApiResponse.success(null, "참여 중인 활성 배틀방이 없습니다."));
        } else {
            return ResponseEntity.ok(CommonApiResponse.success(battleRoom));
        }
    }
