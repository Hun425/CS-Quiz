...
public class BaseUrlConfiguration {
    ...
    public OAuth2AuthorizationRequestResolver oAuth2AuthorizationRequestResolver(
        ...
        log.debug("Configuring OAuth2AuthorizationRequestResolver...");
        // 기본 OAuth2 인증 요청 리졸버 생성 (기본 인증 엔드포인트 경로: /api/oauth2/authorize)
        DefaultOAuth2AuthorizationRequestResolver resolver =
                new DefaultOAuth2AuthorizationRequestResolver(
                        clientRegistrationRepository, "/api/oauth2/authorize"); // TODO: 인증 엔드포인트 경로 상수화 또는 설정 분리 고려

        // baseUrl이 설정되어 있고 비어있지 않은 경우에만 커스터마이저 적용
        if (baseUrl != null && !baseUrl.trim().isEmpty()) {
            log.info("Applying base URL customization for OAuth2 requests: {}", baseUrl);
            resolver.setAuthorizationRequestCustomizer(customizeAuthorizationRequest());
        } else {
            log.debug("Base URL ('app.base-url') is not configured, skipping OAuth2 request customization.");
        }
    ...
    private Consumer<OAuth2AuthorizationRequest.Builder> customizeAuthorizationRequest() {
        return builder -> {
            ...
            log.trace("Original redirect URI in builder: {}", originalRedirectUri);
            ...
            if (originalRedirectUri != null && !originalRedirectUri.startsWith(baseUrl)) {
                ...
                log.debug("Customizing redirect URI: Original='{}', New='{}'", originalRedirectUri, newRedirectUri);
                // 수정된 리다이렉션 URI를 빌더에 다시 설정
                builder.redirectUri(newRedirectUri);
            } else if (originalRedirectUri != null) {
                log.trace("Redirect URI already starts with base URL or is null, no customization needed.");
            }
        };
    }
}
