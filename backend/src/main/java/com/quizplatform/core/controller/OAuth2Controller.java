...
public class OAuth2Controller {
    ...
    @Operation(
            summary = "소셜 로그인 시작",
            description = "선택한 제공자(Google, Github, Kakao)를 통한 소셜 로그인을 시작합니다."
    )
    @GetMapping("/authorize/{provider}")
    public ResponseEntity<Void> startOAuth2Login(
            @PathVariable
            @Parameter(description = "소셜 로그인 제공자 (google, github, kakao)")
            String provider,
    ...
    @Operation(
            ...
            description = "소셜 로그인 인증 후 콜백을 처리하고 JWT 토큰을 발급합니다."
    )
