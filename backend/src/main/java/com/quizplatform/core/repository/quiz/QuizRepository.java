...
public interface QuizRepository extends JpaRepository<Quiz, UUID>, CustomQuizRepository {
    ...
    int countByTagIds(@Param("tagIds") Set<UUID> tagIds);
    ...
    Double calculateAverageDifficultyByTagIds(@Param("tagIds") Set<UUID> tagIds);

    // 특정 사용자가 생성한 퀴즈 목록
    @Query("SELECT q FROM Quiz q WHERE q.creator.id = :creatorId")
    Page<Quiz> findByCreatorId(@Param("creatorId") UUID creatorId, Pageable pageable);
    ...
    Optional<Quiz> findQuizForDaily(
            @Param("recentTagIds") Set<UUID> recentTagIds,
