...
// TODO: UserPrincipal, JwtTokenProvider, AuthResponse 등의 최종 위치 확인 후 import 경로 재검토 필요
import com.quizplatform.common.adapter.config.security.UserPrincipal;
import com.quizplatform.common.adapter.config.security.jwt.JwtTokenProvider;
// TODO: AuthResponse 클래스를 찾을 수 없습니다. 이 클래스를 생성하거나 올바른 경로를 찾아야 합니다.
// import com.quizplatform.common.adapter.dto.AuthResponse;
...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        if (response.isCommitted()) {
            log.debug("Response has already been committed."); // logger -> log
            return;
        }
        ...
        log.debug("OAuth2 authentication successful for user: {}", userPrincipal.getEmail()); // logger -> log

        // 2. JWT Access Token 및 Refresh Token 생성
        // TODO: JwtTokenProvider 위치 확인
        String accessToken = tokenProvider.generateAccessToken(authentication);
        String refreshToken = tokenProvider.generateRefreshToken(authentication);
        log.debug("Generated tokens for user: {}", userPrincipal.getEmail()); // logger -> log
        ...
        // TODO: AuthResponse 클래스가 없습니다. 임시로 변수를 직접 정의하여 사용
        /*
        AuthResponse authResponse = AuthResponse.builder()
                ...
                .username(userPrincipal.getUsername()) // UserPrincipal 에 getUsername() 사용
                .build();
        */
        ...
        log.info("Redirecting user {} to target URL: {}", userPrincipal.getEmail(), targetUrl); // 로그 메시지 수정, logger -> log

        // 7. 리다이렉트 수행 (SimpleUrlAuthenticationSuccessHandler의 기능 사용)
        clearAuthenticationAttributes(request); // 이전 인증 속성 정리 (주석 제거)
    ...
    private void saveRefreshToken(Long userId, String refreshToken) {
        ...
        log.debug("Saving refresh token to Redis with key: {}", key); // logger -> log
        try {
            // Redis에 값 설정 (만료 시간 14일 지정)
            // TODO: RedisTemplate 위치 확인
            redisTemplate.opsForValue().set(
                    key,
                    refreshToken,
                    Duration.ofDays(14) // Redis 저장 만료 기간 설정
            );
            log.info("Refresh token saved successfully for user ID: {}", userId); // logger -> log
        } catch (Exception e) {
            log.error("Failed to save refresh token to Redis for user ID {}: {}", userId, e.getMessage(), e); // logger -> log
