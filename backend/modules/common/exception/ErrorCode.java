...
public enum ErrorCode {
    ...
    // TODO: EMAIL_ALREADY_EXISTS 는 user 모듈로 이동 고려
    EMAIL_ALREADY_EXISTS(HttpStatus.CONFLICT, "U004", "이미 존재하는 이메일입니다."),

    // Auth Related Errors (인증/인가 관련 오류)
    AUTHENTICATION_FAILED(HttpStatus.UNAUTHORIZED, "A001", "인증에 실패했습니다."),
    AUTHORIZATION_FAILED(HttpStatus.FORBIDDEN, "A002", "인가(권한)에 실패했습니다."),
    TOKEN_EXPIRED(HttpStatus.UNAUTHORIZED, "A003", "토큰이 만료되었습니다."),
    INVALID_TOKEN(HttpStatus.UNAUTHORIZED, "A004", "유효하지 않은 토큰입니다."),
    OAUTH2_PROCESSING_ERROR(HttpStatus.BAD_REQUEST, "A005", "OAuth2 처리 중 오류가 발생했습니다."),
