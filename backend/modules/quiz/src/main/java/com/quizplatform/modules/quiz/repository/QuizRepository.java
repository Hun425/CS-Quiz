...
public interface QuizRepository extends JpaRepository<Quiz, Long>, CustomQuizRepository {
    ...
    @Query("SELECT AVG(" +
            ...
            "    WHEN com.quizplatform.modules.quiz.domain.DifficultyLevel.BEGINNER THEN 50 " +
            "    WHEN com.quizplatform.modules.quiz.domain.DifficultyLevel.INTERMEDIATE THEN 100 " +
            "    WHEN com.quizplatform.modules.quiz.domain.DifficultyLevel.ADVANCED THEN 150 " +
     ...
     */
    @Query("SELECT q FROM Quiz q " +
            "WHERE q.quizType = com.quizplatform.modules.quiz.domain.QuizType.REGULAR " + // 일반 퀴즈 타입
            "AND q.isPublic = true " + // 공개된 퀴즈
            "AND q.id NOT IN (" + // 서브쿼리: 최근 데일리 퀴즈로 사용된 퀴즈 ID 제외
            "   SELECT dq.id FROM Quiz dq " +
            "   WHERE dq.quizType = com.quizplatform.modules.quiz.domain.QuizType.DAILY " + // 데일리 퀴즈 타입
