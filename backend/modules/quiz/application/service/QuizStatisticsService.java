...
import application.port.in.GetQuizStatisticsUseCase;
import application.port.out.LoadQuestionAttemptPort;
...
public class QuizStatisticsService implements GetQuizStatisticsUseCase {
    ...
    public Optional<QuizStatistics> getQuizStatistics(Long quizId) {
        ...
        // 퀴즈 시도 조회
        List<QuizAttempt> attempts = loadQuizAttemptPort.findByQuizId(quiz.getId(), org.springframework.data.domain.Pageable.unpaged()).getContent();
    ...
    public Optional<QuizStatistics> getUserQuizStatistics(Long userId, Long quizId) {
        ...
        // 사용자의 퀴즈 시도 조회
        List<QuizAttempt> attempts = loadQuizAttemptPort.findByUserIdAndQuizId(userId, quizId, org.springframework.data.domain.Pageable.unpaged()).getContent();
    ...
    private int calculateAverageTime(List<QuizAttempt> attempts) {
        OptionalDouble average = attempts.stream()
                .filter(QuizAttempt::isCompleted)
