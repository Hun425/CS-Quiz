...
public class OAuth2SuccessHandlerImpl extends SimpleUrlAuthenticationSuccessHandler implements OAuth2SuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        // 인증 객체로부터 UserPrincipal 추출
        // 실제 구현에서는 UserPrincipal 클래스를 사용하여 이 부분을 구현
        ...
        AuthResponse authResponse = AuthResponse.builder()
                ...
                .tokenType("Bearer")
                .expiresIn(expiresIn)
                .email(username) // 실제 구현에서는 UserPrincipal에서 이메일 추출
                .username(username) // 실제 구현에서는 UserPrincipal에서 사용자명 추출
                .build();
